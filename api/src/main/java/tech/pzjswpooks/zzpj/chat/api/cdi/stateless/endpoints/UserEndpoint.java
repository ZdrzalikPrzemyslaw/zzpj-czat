...
public class UserEndpoint {
    ...
    public Response searchUserByUsername(SearchUserRequestDto searchUserRequestDto) {
        ...
        try {
            usersEntities = usersManager.searchUserByUsernameRegex(searchUserRequestDto);
        } catch (Exception e) {
            e.printStackTrace();
            return Response.status(Response.Status.BAD_REQUEST).entity(new SearchUserResponseDto(null, new MessageResponseDto(e.getMessage()), false)).build();
        }
        return Response.status(Response.Status.OK).entity(new SearchUserResponseDto(usersEntities, null, true)).build();
    }
